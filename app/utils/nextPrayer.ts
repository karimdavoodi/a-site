...
/**
 * Parse a prayer time string like "5:30 AM", "13:15", or "1:29" into total minutes
 * from midnight. When the format is ambiguous (no AM/PM, no leading zero on hours),
 * uses the prayer name to determine AM vs PM:
 *   - Fajr is always AM
 *   - Dhuhr, Asr, Maghrib, Isha are always PM
 */
export function parsePrayerTimeToMinutes(
  timeStr: string,
  prayerName: string,
): number {
  if (!timeStr) return -1;
  ...
  // Try 12-hour format with explicit AM/PM: "5:30 AM" or "5:30 PM"
  ...
  // Try 24-hour format: "13:15" or ambiguous "1:29"
  const hhMmMatch = trimmed.match(/^(\d{1,2}):(\d{2})$/);
  if (hhMmMatch) {
    let hours = parseInt(hhMmMatch[1], 10);
    const minutes = parseInt(hhMmMatch[2], 10);

    // Disambiguate using prayer name when no AM/PM indicator
...
/**
 * Legacy: generic parser without prayer-name awareness.
 * Prefer parsePrayerTimeToMinutes for prayer times.
 */
 ...
 * If all prayers for today have passed, returns -1.
 *
